...
const setUserToVendor = async()=>{
  ...
  useEffect(()=>{
    ...
    No stores
      ...
      { (stores!=null ||undefined) && stores.map((store:any)=>{
        return (
          ...
          {store.products.map((product:any)=>{
            return <div className="m-10 mx-4 max-w-screen-lg overflow-hidden rounded-xl border shadow-lg md:pl-8">
          <div className="flex flex-col overflow-hidden bg-white sm:flex-row md:h-80">
            ...
            <div className="order-first ml-auto h-48 w-full bg-gray-700 sm:order-none sm:h-auto sm:w-1/2 lg:w-2/5">
              <Image alt='any image' className="h-full w-full object-cover" src={`${product.imageurl}`} loading="lazy" />
            </div>
          </div>
        </div>
      ...
      :
      <>
      {profile.communityId._id === null || undefined ?
